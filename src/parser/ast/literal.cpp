...
String IntLiteralAST::emit_cst(){
    return value;
}

String IntLiteralAST::emit_ll(int*, String inp){
    return rinsert(getValue(), inp);
}

AST* IntLiteralAST::parse(std::vector<lexer::Token> tokens, int, symbol::Namespace*, String){
    if (tokens.size() < 1 || tokens.size() > 2) return nullptr;
        ...
        else if ((tokens[0].type == lexer::Token::Type::SUB || tokens[0].type == lexer::Token::Type::NEC) && tokens.size() == 2 && tokens[1].type == lexer::Token::Type::INT){
            return new IntLiteralAST(32, String("-") + tokens[1].value, true, tokens);
        }
    return nullptr;
}
...
void IntLiteralAST::forceType(String type){
    ...
    if (expected_int){
        ...
        if(value[0] == '-' && !sig) parser::error("Sign mismatch", {tokens[0],tokens[1]},String("Found a signed value (expected \e[1m") + type + "\e[0m)", 45);
        tsigned = sig;
        this->bits = bits;

    }
    else parser::error("Type mismatch", tokens,String("expected a \e[1m") + type + "\e[0m, found int", 17, "Caused by");
}
...
String BoolLiteralAST::emit_ll(int*, String inp){
    return rinsert(getValue(), inp);
}

String BoolLiteralAST::emit_cst(){
    return value? "true" : "false";
}
AST* BoolLiteralAST::parse(std::vector<lexer::Token> tokens, int, symbol::Namespace*, String){
    if (tokens.size() == 1){
...
void BoolLiteralAST::forceType(String type){
    if (type != "bool") parser::error("Type mismatch", tokens ,String("expected a \e[1m") + type + "\e[0m, found bool", 17, "Caused by");
}
...
String FloatLiteralAST::emit_ll(int*, String inp){
    return rinsert(getValue(), inp);
}

String FloatLiteralAST::emit_cst(){
    return value;
}

String FloatLiteralAST::getLLType(){
...
AST* FloatLiteralAST::parse(std::vector<lexer::Token> tokens, int, symbol::Namespace*, String){
    ...
    if (tokens[0].type == lexer::Token::Type::ACCESS && tokens[1].type == lexer::Token::Type::INT){
        return new FloatLiteralAST(32, (sig? String("-0.") : String("0.")) + tokens[1].value + "e00", t);
    }
...
void FloatLiteralAST::forceType(String type){
    ...
    else parser::error("Type mismatch", tokens ,String("expected a \e[1m") + type + "\e[0m, found float" + std::to_string(bits), 17, "Caused by");
}
...
String CharLiteralAST::emit_ll(int*, String inp){
    return rinsert(getValue(), inp);
}

AST* CharLiteralAST::parse(std::vector<lexer::Token> tokens, int, symbol::Namespace*, String){
    if (tokens.size() != 1) return nullptr;
    if (tokens[0].type == lexer::Token::Type::CHAR){
        if (tokens[0].value.size() == 2){
            parser::error("Empty char", tokens, "This char value is empty. This is not supported. Did you mean '\\u0000' ?", 578);
            return new AST();
        }
        ...
        parser::error("Invalid char", tokens, "This char value is not supported. Chars are meant to hold only one character. Did you mean to use \"Double quotes\" ?", 579);
        return new AST();
    }
    return nullptr;
}

String CharLiteralAST::getValue(){
    if (this->value == "'\\n'") return "u0x000A";
...
void CharLiteralAST::forceType(String type){
    if (type != "char"){
        parser::error("Type mismatch", tokens ,String("expected a \e[1m") + type + "\e[0m, found char", 17, "Caused by");
    }
}
...
AST* StringLiteralAST::parse(std::vector<lexer::Token> tokens, int, symbol::Namespace*, String){
    if (tokens.size() != 1) return nullptr;
...
String StringLiteralAST::emit_ll(int*, String inp){
    return rinsert(getValue(), inp);
}

String StringLiteralAST::getValue(){
    /*if (this->value == "'\\n'") return "\"\\00\\0A\"";
...
void StringLiteralAST::forceType(String type){
    if (type != "String"){
        parser::error("Type mismatch", tokens ,String("expected a \e[1m") + type + "\e[0m, found String", 17, "Caused by");
    }
}
