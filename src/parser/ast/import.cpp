...
sptr<AST> ImportAST::parse(std::vector<lexer::Token> tokens, int,
    ...
    if (tokens.size() < 2)
        return nullptr;
    if (tokens.at(0).type == lexer::Token::IMPORT) {
        uint32 i2 = 0;
        auto buffer = subvector(tokens, 1, 1, tokens.size());
        ...
        for (lexer::Token a : buffer){
            if (last == lexer::Token::SUBNS && (a.type == lexer::Token::DOTDOT || a.type == lexer::Token::ID)){
                ...
                if (buffer.size() > i2 + 2){
                    if (buffer.at(i2 + 1).type == lexer::Token::Type::ID) {
                        if (!(buffer.at(i2 + 2).type ==
                ...
                if (buffer.size() > i2 + 3 &&
                    ...
                    buffer.at(buffer.size() - 1).type == lexer::Token::Type::BLOCK_CLOSE) {
                ...
                if (buffer.size() > i2+1 && buffer.at(i2+1).type == lexer::Token::Type::END_CMD && i2 > 0){
                    break;
                } else {
    ...
    for (uint32 i = 0; i < tokens.size() - 1; i++) {
        if (tokens.at(i).type == lexer::Token::IMPORT && i != 0) {
            parser::error(
